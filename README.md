...
### 5. 🗺️ Adventure Map — 阅读冒险地图
一条会流动的动画小路，12 个关卡地标（🌱 First Step → 🏰 Library Castle → 🐉 Dragon Peak → 👑 Reading Royalty）。
**1 本书 = 1 步 · 1 个闯关 = 1 步 · 1 个故事 = 2 步**。走到的地标会发光跳动，下一个目标有脉冲光圈，小伙伴走在路上。

### 6. 🐣 Reading Buddy — 会长大的阅读伙伴
一颗会摇晃的蛋，随着 💎 宝石增加逐级进化：
🥚 Mystery Egg → 🐣 Hatchling (30) → 🐥 Fluffy (80) → 🦉 Wise Owl (160) → 🐉 Book Dragon (300)
点它会跳起来、冒爱心并说话，下面有进化图鉴和成长进度条。

### 7. ✨ My Story Book
她说出来的每一个故事都会被写下来存在这里，可以让电脑 🔊 念回给她听。

### 🎬 动画细节（v2）
- 书封面：渐变会流动、emoji 上下漂浮、✨ 星星闪烁，鼠标移上去书会跳起来
- 首页有 **Ollie 🦉 猫头鹰导师**：会眨眼、会拍翅膀，点一下给一条阅读小贴士（还会念出来）
- 背景有 14 个缓缓上升的漂浮书本/星星
- 🎡 **Book Wheel**：书架页点「Surprise me!」转盘随机抽一本书
- 通关/记录书本有彩带爆炸，徽章解锁有弹跳，页面切换有滑入动画
